...
function LatexCompilerPage() {
  ...
  return (
    <div>
      ...
      <LatexCompiler
        apiUrl="http://localhost:5001" // Your tectonic microservice URL
...
```env
# .env
REACT_APP_TECTONIC_API_URL=http://localhost:5001
```

Then update your components to use the environment variable:

```tsx
const apiUrl = process.env.REACT_APP_TECTONIC_API_URL || 'http://localhost:5001';
```
